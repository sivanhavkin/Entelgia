...
They apply to pull request review, code suggestions, code generation, and repository-aware chat.

The most important rule is:

**Preserve Entelgia’s architecture.**
Do not flatten, simplify away, or merge core systems unless explicitly asked.
...
Fixy should remain distinct from dialogue generation.
Fixy is not just another speaker. Fixy is a meta-regulator.
...
Emotion is not decorative metadata.
Emotion affects routing, salience, and storage.
...
Do not turn BehaviorCore into a duplicate of prompt text.
It should remain a decision-affecting subsystem.
...
Do not reduce ConsciousCore to a logger.
It represents internal state relevant to dialogue evolution.
...
1. choose active speaker
2. build prompt context
3. retrieve relevant memory
4. include internal state
5. optionally include research context
6. call LLM
7. post-process response
8. update state
9. store memory
10. evaluate need for Fixy intervention
11. continue loop
...
Expected pipeline:
1. detect trigger
2. check cooldown / gating
3. build query
4. sanitize query
5. perform search
6. fetch pages
7. summarize / compress
8. inject research context into next prompt
...
For model guidance:
- prefer phrasing like “Phi-3 class or stronger recommended”
  if smaller models do not reliably sustain the architecture
...
2. module responsibility correctness
3. unintended coupling
4. hidden regressions
5. dialogue stability
6. memory / intervention correctness
7. search pipeline correctness
...
1. explain the existing data flow
2. identify the narrowest safe change
3. propose a minimal patch
4. mention risks explicitly
