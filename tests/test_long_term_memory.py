...
class TestDefenseMechanism:
    ...
    def test_suppression_dangerous_content(self, defense):
        """Content with suppression trigger words is recognized."""
        flags = defense.analyze(
            content="There is a hidden secret that must stay private",
            emotion="neutral",
            emotion_intensity=0.2,
            importance=0.5,
        )
...
class TestSelfReplication:
    ...
    def test_find_patterns_recurring_english_keywords(self, replication):
        """English words longer than 4 chars are detected as patterns."""
        memories = [
            {"content": "philosophy is the love of wisdom and truth"},
            {"content": "wisdom and knowledge form the basis of philosophy"},
        ]
        patterns = replication.find_patterns(memories)
        # 'wisdom' and 'philosophy' appear in both memories
