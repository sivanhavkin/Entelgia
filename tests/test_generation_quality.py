...
class TestContextManagerContractInjection:
    ...
    def test_agent_prompts_are_structurally_distinct(self):
        ...
        assert (
            "ATHENA ROLE GOAL" not in soc_prompt and "ATHENA CONTRACT" not in soc_prompt
        )
        assert "ATHENA" in ath_prompt
        assert (
            "SOCRATES ROLE GOAL" not in ath_prompt
            and "SOCRATES CONTRACT" not in ath_prompt
        )
        assert "FIXY" in fxy_prompt
        assert (
            "SOCRATES ROLE GOAL" not in fxy_prompt
            and "SOCRATES CONTRACT" not in fxy_prompt
        )
