...
from entelgia import DialogueEngine, ContextManager, InteractiveFixy, get_persona, format_persona_for_prompt
...
def test_seed_variety():
    ...
    for turn in range(1, 21):
        dialog = [{"role": "Socrates", "text": "test", "emotion": "neutral"}] * min(turn, 5)
...
def test_context_enrichment():
    ...
    # Create mock data
    drives = {"id_strength": 6.0, "ego_strength": 5.5, "superego_strength": 5.0, "self_awareness": 0.6}
    debate_profile = {"style": "integrative, Socratic", "dissent_level": 4.5}
    
    dialog_tail = [
        {"role": "Socrates", "text": f"This is turn {i}. " * 50}
        for i in range(10)
    ]
    
    stm = [
        {"text": f"Thought {i}. " * 20, "emotion": "curious"}
        for i in range(10)
    ]
...
def test_fixy_interventions():
    ...
    if should and reason == "circular_reasoning":
        ...
        print(f"  ✗ Failed to detect circular reasoning (should={should}, reason={reason})")
...
def test_persona_formatting():
    ...
    checks = {
        ...
        "Fixy has intervention triggers": len(fixy.get("intervention_triggers", [])) >= 3,
