...
- **`tests/test_behavioral_rules.py`** expanded from 18 to **29 tests** (+11). Added `TestRuleLHAthenaLimbicHijack` (5 tests: anger rule fires during hijack, mentions harsh language, takes priority over Rule B, silent when hijack is off, Socrates hijack does not trigger Athena rule) and `TestRuleSCSocratesAnxiety` (6 tests: anxiety rule fires when SuperEgo dominant, mentions nervousness, fires without random gate, absent when SuperEgo not dominant, takes priority over Rule A, Athena SuperEgo-dominant does not trigger SC rule).
