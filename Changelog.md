...
- **Rule AI-tension — Athena graduated irritation (id 7.0–8.5)** — new fallback behavioral rule. When Athena's `id_strength` is in `[7.0, 8.5)` and no higher-priority rule (LH, B) applies, a graduated irritation instruction is injected. The phrasing escalates from *"subtle undercurrent of irritation"* (id near 7.0) through *"growing frustration"* (mid range) to *"clear irritation and barely-contained anger"* (id near 8.5), mirroring the pre-hijack emotional build-up described by the limbic-hijack model. This rule fires only when limbic hijack is inactive and conflict-based Rule B does not trigger.
...
- **`tests/test_behavioral_rules.py`** expanded from 18 to **55 tests** (+37). Added `TestRuleLHAthenaLimbicHijack` (5 tests: anger rule fires during hijack, mentions harsh language, takes priority over Rule B, silent when hijack is off, Socrates hijack does not trigger Athena rule), `TestRuleSCSocratesAnxiety` (6 tests: anxiety rule fires when SuperEgo dominant, mentions nervousness, fires without random gate, absent when SuperEgo not dominant, takes priority over Rule A, Athena SuperEgo-dominant does not trigger SC rule), `TestRuleAITensionAthena` (7 tests: tension fires at id=7, fires at id=8, fires at id=8.4, absent at id=8.5, absent when hijack active, absent for Socrates, instructs sharpened language), `TestRuleAICuriosoAthena` (6 tests: fires at default id=5, fires at id=6.9, absent at id=7, mentions wonder, absent for Socrates, LH takes priority), `TestRuleSIAnxiousSocrates` (6 tests: fires at id=7, fires at id=8, absent at id=8.5, mentions guardedness, SC takes priority, absent for Athena), and `TestRuleSISkepticSocrates` (7 tests: fires at default id, fires at id=6.9, absent at id=7, mentions principled disagreement, absent for Athena, SC takes priority, positive framing). Updated two pre-existing `test_returns_empty_at_or_below_6` tests to assert that conflict-based Rules A/B specifically do not fire (rather than asserting an empty string) — these tests correctly reflect that id-based fallback rules may now fire when id is in range.
...
- **`tests/test_web_research.py`** expanded from 181 to **192 tests** (+11). Added tests for detailed per-branch logging in `maybe_add_web_context` (seed branch source type, detected trigger, preview truncation; dialogue branch source type, role, detected trigger; Fixy reason branch source type, preview; query-build branch logging for all three branches), session-start trigger origin discrimination, per-query cooldown deduplication (same sanitized query suppressed within cooldown, different queries independent, cooldown expiry, clear resets state, same sanitized query from different `seed_text` is still suppressed), and integration of blacklist/per-query cooldown inside `maybe_add_web_context`.
- **Total test count**: 721 → **749 tests** (749 passed, 0 skipped) across 23 suites.
