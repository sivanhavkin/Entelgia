...
- **`_FINAL_STAGE_PERSONA_NOTES`** — new module-level dict mapping `"Socrates"`, `"Athena"`, and `"Fixy"` to per-agent identity instructions used in the Stage 2 transformation prompt.
...
- **Behavior-driven generation policy** — new module-level constants injected at generation time: `LLM_OUTPUT_CONTRACT` (one concrete claim + one supporting reason/mechanism, max 2–3 sentences, no visible labels), `LLM_BEHAVIORAL_CONTRACT_SOCRATES`, `LLM_BEHAVIORAL_CONTRACT_ATHENA`, and `LLM_BEHAVIORAL_CONTRACT_FIXY`. Per-agent behavioral contracts define output logic and allowed moves, not tone labels.
- **`BANNED_RHETORICAL_TEMPLATES`** — module-level list of banned rhetorical scaffolding phrases (`"we must consider"`, `"it is important to recognize"`, etc.) used to build both the forbidden-phrase instruction and the quality gate.
- **Output quality gate** — `output_passes_quality_gate(text)` returns `False` when ≥ 2 `_QUALITY_GATE_PATTERNS` hits are found in a draft. When the gate fails, one regeneration attempt is made with a stricter prompt that names the specific banned patterns found; if the second draft also fails, output proceeds to avoid an infinite loop.
...
- **Grammar repair in `TextHumanizer`** — `_repair_grammar()` method (gated by `grammar_repair_enabled` and `repair_broken_openings` init flags) detects and corrects broken sentence openings using `_build_broken_patterns()`. Controlled by new `Config` fields `humanizer_grammar_repair_enabled` (default `True`) and `humanizer_repair_broken_openings` (default `True`).
...
- **`Config.humanizer_enabled` default changed to `False`** — TextHumanizer post-processing is now OFF by default. Enable via `Config(humanizer_enabled=True)`.
...
- **Emotion inference moved to end of `speak()`** — `emotion.infer()` is now called after LLM generation and all post-processing steps rather than before. This eliminates a redundant LLM call that was stalling `speak()` when humanizer and web research were both disabled (the previous code path triggered inference twice in that scenario).
...
- **`tests/test_affective_ltm_integration.py`** — new suite with **24 tests** covering affective LTM retrieval integration: debug toggle, min-score threshold, supplement injection into prompt context, empty supplement when disabled, per-agent emotion weighting.
- **`tests/test_generation_quality.py`** — new suite with **75 tests** covering `output_passes_quality_gate`, `_strip_scaffold_labels`, `LLM_OUTPUT_CONTRACT` phrase cleanliness, `LLM_FORBIDDEN_PHRASES_INSTRUCTION` (including new banned phrases), per-agent behavioral contracts, and output contract prose requirements.
- **`tests/test_stabilization_pass.py`** — new suite with **55 tests** covering memory topic filter (`_score_memory_topic_relevance`), cluster wallpaper penalty, Fixy role-aware compliance, web trigger multi-signal gate, topic anchor configurable fields, self-replication topic gate, and debug flag forwarding.
- **`tests/test_text_humanizer_integration.py`** — new suite with **41 tests** covering `TextHumanizer` grammar repair (broken opening detection, repair correctness, `repair_broken_openings` flag, `grammar_repair_enabled` flag), humanizer pipeline integration, and `show_humanizer_debug` flag.
- **`tests/test_topic_enforcer.py`** — new suite with **41 tests** covering `compute_topic_compliance_score`, `compute_fixy_compliance_score`, `get_cluster_wallpaper_terms`, `get_topic_distinct_lexicon`, `build_soft_reanchor_instruction`, `ACCEPT_THRESHOLD`, and `SOFT_REANCHOR_THRESHOLD`.
- **`tests/test_web_research.py`** expanded to **202 tests** (+10 from 192). Added tests for `_count_strong_trigger_hits`, `_has_uncertainty_or_evidence_signal`, multi-signal gate gating logic, `_HIGH_VALUE_KEYWORDS` qualification, and rhetorical framing word exclusion from concept scoring.
- **`tests/test_web_tool.py`** expanded to **26 tests** (+8 from 18). Added tests for retry-on-transient-error, response text extraction, and additional blacklist edge cases.
- **`tests/test_topic_anchors.py`** expanded to **60 tests** (+1 from 59). Added test for configurable `topic_anchor_max_forbidden_items`.
- **`tests/test_ablation_study.py`** expanded to **28 tests** (+1 from 27).
- **`tests/test_behavioral_rules.py`** expanded from 18 to **71 tests** (+53). Added `TestRuleLHAthenaLimbicHijack` (5 tests), `TestRuleSCSocratesAnxiety` (6 tests), `TestRuleAITensionAthena` (8 tests: +1 `test_tension_rule_mentions_impulsive`), `TestRuleAICuriosoAthena` (6 tests), `TestRuleSIAnxiousSocrates` (6 tests), `TestRuleSISkepticSocrates` (7 tests), `TestRuleIDLow` (7 tests: fires for Athena, fires for Socrates, fires at id=0, absent at id=5.0, mentions reserved/passive, LH takes priority, SC takes priority), and `TestRuleSELow` (8 tests: fires for Athena, fires for Socrates, fires at sup=0, absent at sup=5.0, mentions risk-taking, ID-low takes priority, SC/SE-low interaction, LH takes priority). Updated two pre-existing `test_returns_empty_at_or_below_6` tests.
- **`tests/test_superego_critique.py`** expanded from 21 to **28 tests** (+7). Added `TestSuperEgoConsecutiveStreakLimit` (3 tests: first two turns apply rewrite, third turn suppresses, counter resets after non-critique turn), `TestSuperegoCritiqueAtExtreme` (4 tests: tight margin fires at barely-dominant extreme SuperEgo, normal margin suppresses barely-dominant SuperEgo, low conflict fires with extreme `conflict_min`, normal `conflict_min` suppresses low conflict), and `TestSocratesAnxietyDuringCritique` (3 tests: last emotion is fear when critique fires, not fear when critique does not fire, critique prompt mentions anxious tone).
- **`tests/test_limbic_hijack.py`** expanded from 15 to **20 tests** (+5). Added `TestLimbicHijackExtremeId` (2 tests: activates with moderate intensity at extreme Id, no activation with moderate intensity at normal Id) and `TestAthenaLimbicHijackAnger` (3 tests: last emotion is anger during hijack, behavioral rule contains anger instruction, non-Athena agent does not get anger rule).
- **`tests/test_drive_correlations.py`** expanded from 21 to **28 tests** (+7). Added `TestAgentBiasedDriveReversion` (7 tests: Athena id drifts above neutral over turns, Socrates superego drifts above neutral over turns, generic agent id stays near neutral, Athena id bias drains ego, Socrates superego bias drains ego, Athena id extreme-high reverts faster, Socrates superego extreme-low reverts faster).
- **Total test count**: 749 → **1043 tests** (1043 passed, 0 skipped) across **29 suites** (27 collecting successfully).
