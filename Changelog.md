...
- **`[FATIGUE]` log line every turn** — emitted at `INFO` level inside `Agent.speak()` on every call: `[FATIGUE] agent=Socrates level=0.68 state=medium energy=43.0`. Provides a per-turn observable trace of the agent's fatigue state, independent of any structural loop signal.
...
- **`print_meta_state()` Energy line extended** — the `Energy: …  Conflict: …` line in the `[META]` block now also shows `Fatigue: …  State: …` so operators can read all energy-related diagnostics in one place without scrolling to a separate log line.
...
- **`tests/test_fatigue_tagging.py`** — new suite with **42 tests** covering: `_compute_fatigue` score and state for all three energy regimes (above threshold, 35–60 range, below dream threshold); monotonicity of fatigue as energy decreases; exact formula verification at boundary and midpoint values; clamp invariant (score always in [0.0, 1.0]); `Agent.__init__` field initialisation (`_last_fatigue`, `_last_fatigue_state`); and explicit confirmation that `_compute_fatigue` is a pure function with no side-effects on loop/stagnation flags.
- **Total test count**: **1498 tests** across **35 suites**.
